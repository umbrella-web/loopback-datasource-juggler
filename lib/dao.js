...
//find _req in args
//_req may passed from strong-remoting module (loopback/node_modules/strong-remoting/lib/shared-method.js SharedMethod.prototype.invoke)
//_req may passed from custom executing Models methods (example Model.find({_req: _req}))
//also clean all args from _req (arg data with _req may cause bodyParser error)
function fnFindReqInArgs(arguments) {
  var args = Array.prototype.slice.call(arguments)
    , req = null;
  for(var i = 0, l = args.length; i < l; i++) {
    if(args[i] && typeof args[i] === 'object' && args[i]._req) {
      req = args[i]._req;
      delete args[i]._req;
    }
  }
...
DataAccessObject.create = function (data, options, cb) {
  ...
  var _req = fnFindReqInArgs(arguments);
...
DataAccessObject.find = function find(query, options, cb) {
  ...
  if (options.notify === false) {
    ...
    if(query.count) {
      var aCount = [];
      //only string or plain object to arr
      if (typeof query.count === 'string' || (typeof query.count === 'object' && !Array.isArray(query.count))) {
        query.count = [query.count];
      }

      if(Array.isArray(query.count)) {
        for(var i = 0, l = query.count.length; i < l; i++) {
          var mItem = query.count[i]
            , oItem = {
              relation: ((mItem && typeof mItem === 'object') ? mItem.relation : mItem) || null,
              where: (mItem && typeof mItem === 'object' && Object.keys(mItem).length) ? mItem.where : null
            };

          if(oItem.relation) {
            if (self.settings.relations[oItem.relation]) {

              //copy settings
              oItem.settings = util._extend({}, self.settings.relations[oItem.relation]);
              oItem.settings.name = oItem.relation;

              aCount.push(oItem);
            } else {
              console.log('SYNTAX ERROR: `count[%d].relation`(%s) not a relation!', i, oItem.relation);
            }
          } else {
            console.log('SYNTAX ERROR: `count[%d]` have invalid data');
          }

        }
      } else {
        console.log('SYNTAX ERROR: `count` parameter should be string or array or object')
      }
      if(aCount.length)
        query.count = aCount;
      else
