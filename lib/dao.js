 ...
 * @private
 */
DataAccessObject._coerce = function (where, sThroughRelation) {
  ...
  if(where.self || where.through) {
    where.self && self._coerce(where.self);
    if(typeof where.through === 'object') {
      for(var relation in where.through) {
        self._coerce(where.through[relation], relation);
      }
    }

    where.through = throughDefinition.call(self, where.through); //get definition of related model
  ...
  if(!sThroughRelation)
    ...
    var oThroughRelation = self.settings.relations[sThroughRelation]
      , sThroughModel
      , oThroughModel;
    oThroughRelation && (sThroughModel = oThroughRelation.model);
    sThroughModel && (oThroughModel = self.dataSource.models[sThroughModel]);
    oThroughModel && (props = oThroughModel.definition.properties);
  }
  for (var p in where) {
...
//get model data for relation where: { through: {items: /filter/, invoices: /filter/ } } (items, invoices - relations)
function throughDefinition(oThrough) {
  var oStructuredThrough = {},
      self = this;
  if(typeof oThrough !== 'object') {
    console.log('SYNTAX ERROR: `where.through` parameter should be object!');
  } else {
    for(var key in oThrough) { //key = name of relation
      var oWhere = oThrough[key]; //save filter
      if(typeof oWhere !== 'object') {
        console.log('SYNTAX ERROR: `where.through.%s` parameter should be object!', key);
      } else if(oWhere.params && oWhere.where){
        oStructuredThrough[key] = oWhere; //already structured (sometimes trough filter use this funct twice or more), save
      } else{
        var oSett = self.settings.relations[key]; //git setting for relation (type, name ...)
        if(typeof oSett !== 'object' || !oSett.through) {
          console.log('SYNTAX ERROR: `where.through.%s` not a relation, or not a hasManyThrough!', key);
        } else {

          var settKeys = Object.keys(oSett)
            , iSett = settKeys.length
            , oCopiedSett = {};
          while (iSett--) {
            oCopiedSett[settKeys[iSett]] = oSett[settKeys[iSett]];
          }
          oCopiedSett.name = key; //copy all setting and add in this object name of relation

          var whereKeys = Object.keys(oWhere)
            , iWhere = whereKeys.length
            , oCopiedWhere = {};
          while (iWhere--) {
            oCopiedWhere[whereKeys[iWhere]] = oWhere[whereKeys[iWhere]];
          } //copy where filter, for avoid recursion


          oStructuredThrough[key] = { //copy all setting and add in this object name of relation
            where: oCopiedWhere,
            params: oCopiedSett
          };
  ...
  //query.where.through = oWhere;
  return oStructuredThrough;
}
...
DataAccessObject.find = function find(query, options, cb) {
  ...
  if (options.notify === false) {
    ...
    //get required info for create sql query
    if(query.count) {
      var aCount = [];
      if (typeof query.count === 'string' && query.count.length > 0) { //only string to arr
        query.count = [query.count];
      }

      if(Array.isArray(query.count)) {
        for(var i = 0, l = query.count.length; i < l; i++) {
          if (self.settings.relations[query.count[i]]) {
            var sRelationName = query.count[i]
              , oSettings = self.settings.relations[query.count[i]]
              , oCopiedSettions = {};

            for(var k in oSettings) {
              oCopiedSettions[k] = oSettings[k];
            }

            oCopiedSettions.name = sRelationName;

            aCount.push(oCopiedSettions);
          } else {
            console.log('SYNTAX ERROR: `count.%s` not a relation!', query.count[i]);
          }
    ...
    if(typeof query.where === 'object' && query.where.through) {

      query.where.through = throughDefinition.call(self, query.where.through); //get defenition of related model
