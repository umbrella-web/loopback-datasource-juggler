...
function mergeQuery(base, update, spec) {
  ...
  var oThrough //through filter, example {through: {..}, self: {..}, name: 'test'}, can be in base or update
    , oSelf //custom self filter, example {through: {..}, self: {..}, name: 'test'}, can be in base or update
    , oCopU //where filter {name: 'test', groupId: 10}, also can contain through or self filter
    , oCopB //where filter {name: 'test', groupId: 10}, also can contain through or self filter
    , oOther = {}; //save other data, if find through or self filter and additional data, example {self : {...}, name: 'test'}, name - additional data


  //self or through filter can be in base or update arg, first save all data in associated vars
  if(base.where && Object.keys(base.where).length > 0) {//if base where exist
    oCopB = base.where;

    if(oCopB.through && Object.keys(oCopB.through).length > 0 || oCopB.self && Object.keys(oCopB.self).length > 0) { //if through or self in base arg

      oCopB.self && Object.keys(oCopB.self).length > 0 && (oSelf = oCopB.self); //save self to oSelf
      oCopB.through && Object.keys(oCopB.through).length > 0 && (oThrough = oCopB.through); //save through to oThrough

      oSelf && delete oCopB.self;
      oThrough && delete oCopB.through;

      //saved other data, example {self: {}, through: {}, name: 'test', groupId: 'test'} - name, groupId - other data
      for(var kB in oCopB) {
        if(kB === 'through' || kB === 'self')
          continue;
    ...
    if(update.where && Object.keys(update.where).length > 0){
      ...
      if(oCopU.through && Object.keys(oCopU.through).length > 0 || oCopU.self && Object.keys(oCopU.self).length > 0) { //if through or self in update arg

        oCopU.self && Object.keys(oCopU.self).length > 0 && (oSelf = oCopU.self); //save self to oSelf
        oCopU.through && Object.keys(oCopU.through).length > 0 && (oThrough = oCopU.through); //save through to oThrough

        oSelf && delete oCopU.self;
        oThrough && delete oCopU.through;

        //saved other data
        for(var kU in oCopU) {
          if(kU === 'through' || kU === 'self')
            continue;
  ...
  } else if(update.where && Object.keys(update.where).length > 0) { //if update where exist
    oCopU = update.where;

    if(oCopU.through && Object.keys(oCopU.through).length > 0 || oCopU.self && Object.keys(oCopU.self).length > 0) { //if through or self in update arg

      oCopU.self && Object.keys(oCopU.self).length > 0 && (oSelf = oCopU.self); //save self
      oCopU.through && Object.keys(oCopU.through).length > 0 && (oThrough = oCopU.through); //save through
  ...
  //check dirty other data, and if emty, then delete
  ...
  //merge all data in base.where = {self: (self + other data), through: through}
  if(oSelf || oThrough) {
    base.where = {
      self: oSelf || {},
      through: oThrough || {}
    ...
    if(oCopU && oCopB) {
      ...
      base.where = oCopU || oCopB
  }

  //added other data to self
  if(oOther) {
    if(!base.where)
      base.where = {};
    ...
    if (base.where.self)
      for(var k in oOther)
        base.where.self[k] = oOther[k];
    else
      base.where.self = oOther;
  }
